...
const ASSEMBLY_CANDIDATE_SELECTIONS_KEY = 'assemblyCandidateSelections';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (roleStatusSelect) {
        ...
        roleStatusSelect.addEventListener('change', function(e) {
            ...
            setDefaultUseEndDateForOnlineToUsedStandby(e.target.value);
        });
    }
    if (roleNameInput) {
...
function getUseStartDateInputValue(value) {
    return normalizeDateInputValue(normalizeUseStartDate(value));
}
...
function updateStatusPreview(selectEl) {
    ...
    updateUseStartDateFieldState(selectedStatus);
    updateUseEndDateFieldState(selectedStatus);
...
function updateUseStartDateFieldState(status) {
    const field = document.querySelector('.use-start-date-field');
    const input = document.getElementById('role-use-start-date');
    const isAllowed = normalizeRoleStatusValue(status) === ONLINE_STATUS;
...
function isUseEndDateAllowedStatus(status) {
    return [USED_STANDBY_STATUS, REWORK_READY_STATUS, REWORKING_STATUS,
        NEW_READY_STATUS, NEW_INSTALLED_STATUS, NEW_STORAGE_STATUS]
        .includes(normalizeRoleStatusValue(status));
}

function updateUseEndDateFieldState(status) {
    const field = document.querySelector('.use-end-date-field');
    const input = document.getElementById('role-use-end-date');
    const isAllowed = isUseEndDateAllowedStatus(status);
...
function setDefaultUseEndDateForOnlineToUsedStandby(status) {
    if (editingId === null || normalizeRoleStatusValue(status) !== USED_STANDBY_STATUS) return;
    const role = roles.find(item => String(item.id) === String(editingId));
    const input = document.getElementById('role-use-end-date');
    if (role && role.status === ONLINE_STATUS && input && !normalizeDateInputValue(role.useEndDate) && !input.value) {
...
function getWorkshopBoardCandidateKey(candidate) {
    return `assembly-${candidate && candidate.standKey || ''}-${getAssemblyCandidateRoleId(candidate && candidate.usedRole)}`;
}

function getAssemblyCandidateRoleId(role) {
    return String(role && (role.id ?? role.name) || '').trim();
}

function getAssemblyCandidateSelectionKey(standKey, usedRole) {
    return `${String(standKey || '')}::${getAssemblyCandidateRoleId(usedRole)}`;
}

function getStoredAssemblyCandidateSelections() {
    try {
        const rawValue = localStorage.getItem(ASSEMBLY_CANDIDATE_SELECTIONS_KEY);
        const parsedValue = rawValue ? JSON.parse(rawValue) : {};
        return parsedValue && typeof parsedValue === 'object' && !Array.isArray(parsedValue)
            ? parsedValue
            : {};
    } catch (error) {
        console.error('getStoredAssemblyCandidateSelections error:', error);
        return {};
    }
}

function saveAssemblyCandidateSelection(standKey, usedRoleId, installRoleId) {
    const selectionKey = `${String(standKey || '')}::${String(usedRoleId || '')}`;
    const selections = getStoredAssemblyCandidateSelections();

    if (!standKey || !usedRoleId || !installRoleId) {
        return;
    }
    ...
    try {
        ...
        console.error('saveAssemblyCandidateSelection error:', error);
    }
}
...
function getSelectedAssemblyInstallRole(standKey, usedRole, installRoles, selections = getStoredAssemblyCandidateSelections()) {
    const candidates = Array.isArray(installRoles) ? installRoles : [];
...
function getWorkshopBoardSummaryHtml(candidates) {
    ...
    return `
        <div class="workshop-summary-card is-total">
            <span>組替候補 総数</span>
            <strong>${escapeHtml(String(summary.total))}件</strong>
        </div>
    `;
}
...
function getWorkshopBoardFilteredCandidates(candidates) {
    return (Array.isArray(candidates) ? candidates : []).filter(candidate => {
        ...
        if (workshopBoardSelectedOnly && !(candidate && candidate.isSelected)) {
            return false;
        }
...
function getWorkshopBoardCoatingSummary(candidate) {
    const standKey = candidate && candidate.standKey;
    const installRoles = candidate && candidate.selectedInstallRole
        ? [candidate.selectedInstallRole]
        : (Array.isArray(candidate && candidate.newReadyRoles) ? candidate.newReadyRoles : []);
    ...
    const hasCheckNeeded = installRoles.some(role => {
        const status = normalizeRoleStatusValue(role && role.status);
        const coatingStatus = normalizeCoatingStatusValue(role && role.coatingStatus, status, standKey);
        return !coatingStatus || coatingStatus === UNCOATED_STATUS;
    });

    return hasCheckNeeded ? '溶射要確認' : '溶射OK';
}
        ...
        <details class="assembly-selection-menu">
            ...
            <div class="assembly-selection-options">
                ${installCandidates.map(role => {
                    ...
                    const isSelected = candidate && candidate.selectedInstallRole
                        && getAssemblyCandidateRoleId(candidate.selectedInstallRole) === roleId;

                    return `
                        <div class="assembly-selection-option ${isSelected ? 'is-selected' : ''}">
                            <span>${escapeHtml(candidate && candidate.usedRole && candidate.usedRole.name || '-')} → ${escapeHtml(role.name || '-')}</span>
                            <button type="button" onclick="selectAssemblyCandidate('${encodeURIComponent(String(candidate && candidate.standKey || ''))}', '${encodeURIComponent(getAssemblyCandidateRoleId(candidate && candidate.usedRole))}', '${encodeURIComponent(roleId)}')">${isSelected ? '採用中' : '採用'}</button>
                        </div>
                    `;
                }).join('')}
...
function getWorkshopBoardAlertCardHtml(candidate) {
    ...
    return `
        <article class="assembly-alert-card is-${escapeHtml(dueStatus)} ${candidate && candidate.isSelected ? 'is-selected' : 'is-unselected'}">
            ...
            <div class="assembly-arrow-cell">→</div>
            <div class="assembly-roll-cell">
                <span>組む側</span>
...
function getWorkshopAssemblyCandidatesHtml(candidates) {
    ...
    return `
        <div class="assembly-alert-board">
            ${getWorkshopBoardAlertListHtml(candidates)}
        </div>
    `;
}
...
function getWorkshopBoardDetailHtml(candidate) {
    ...
    return `
        <div class="assembly-detail-header">
            <div>
                <span>選択中の組替候補</span>
                <h3>${escapeHtml(candidate.standLabel)}</h3>
            </div>
...
function getWorkshopBoardCandidates(allRoles = roles) {
    ...
    return getAssemblyCandidateItems(allRoles)
        .flatMap(candidate => {
            const usedRoles = Array.isArray(candidate.usedRoles) ? candidate.usedRoles : [];
            const installCandidates = Array.isArray(candidate.newReadyRoles) ? candidate.newReadyRoles : [];
...
function toggleWorkshopBoardSelectedOnly() {
    workshopBoardSelectedOnly = !workshopBoardSelectedOnly;
    updateWorkshopBoard(roles);
}
...
function updateWorkshopBoard(allRoles) {
    ...
    if (selectedToggle) {
        selectedToggle.textContent = workshopBoardSelectedOnly ? 'すべての候補を表示' : '工作課へ表示する候補を確認';
        selectedToggle.setAttribute('aria-pressed', workshopBoardSelectedOnly ? 'true' : 'false');
    }

    countEl.textContent = `${totalCount}件`;

    if (summaryEl) {
        summaryEl.innerHTML = getWorkshopBoardSummaryHtml(filteredCandidates);
    }
...
function addRole() {
    ...
    const roleUseStartDate = roleStatus === ONLINE_STATUS
        ...
        : '';
    const roleUseEndDate = isUseEndDateAllowedStatus(roleStatus)
    ...
    if (roleUseStartDate && roleUseEndDate && roleUseEndDate < normalizeDateInputValue(roleUseStartDate)) {
    ...
    document.getElementById('role-use-start-date').value = '';
    document.getElementById('role-use-end-date').value = '';
    updateInboundPlanPreview();
...
function editRole(id) {
    ...
    document.getElementById('role-use-end-date').value = normalizeDateInputValue(role.useEndDate);
    updateInboundPlanPreview();
...
function updateRole() {
    ...
    const beforeUseEndDate = normalizeDateInputValue(role.useEndDate);
    const comparableNewUseStartDate = normalizeDateInputValue(roleUseStartDate);
    const startsNewUseCycle = roleStatus === ONLINE_STATUS
        && Boolean(comparableNewUseStartDate)
        && Boolean(beforeUseEndDate)
        && comparableNewUseStartDate > beforeUseEndDate;
    const roleUseEndDate = startsNewUseCycle
        ? ''
        : (isUseEndDateAllowedStatus(roleStatus) ? enteredUseEndDate : beforeUseEndDate);
    const roleArrivalDate = isArrivalDateAllowedStatus(roleStatus)
        ? normalizeDateInputValue(document.getElementById('role-arrival-date').value)
        : beforeArrivalDate;
    ...
    if (normalizeRoleStatusValue(beforeStatus) === ONLINE_STATUS && roleStatus !== ONLINE_STATUS) {
        alert(ONLINE_DIRECT_REMOVAL_BLOCK_MESSAGE);
        return;
    }
